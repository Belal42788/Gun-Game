...
public class Gun1 extends AnimListener implements GLEventListener, KeyListener {
    
    String textureNames[] = {"Man1_right.png","Man2_right.png","Man3_right.png","Man4_right.png","B1.png","Background.png","0_Golem_Running_001-1.png",
        "0_Golem_Running_002-1.png","0_Golem_Running_003-1.png","0_Golem_Running_004-1.png"};
...
//        GL gl = glAutoDrawable.getGL();
        ...
        for (int i = 0; i < textureNames.length; i++) {
            try {
                texture[i] = Textures.TextureReader.readTexture(assetsFolderName + "//" + textureNames[i], true);
...
//        gl.glMatriMode(GL.GL_PROJECTION);
//        gl.glLoadIdentity();
...
//        GL gl = glAutoDrawable.getGL();
        ...
        TO_Draw_Soldier1(gl,soldierIndex);

        
        
        //Bullet1
        enemyIndex++;
        enemyIndex = 6 + enemyIndex % 4;
        TO_Draw_Bullets1(gl);
        TO_Move_Bullets1();
...
//        gl.glTranslated(convertX(x), convertY(y), 0);
    ...
    public void TO_Draw_Enemies(GL gl){
        for (int i = 0; i < Max_X; i++) {
            for (int j = 0; j < Max_Y; j++) {
                if (Enemies[i][j] == 1) {
                    TO_Draw_One_Enemy(gl, i, j,enemyIndex);
                }
            }
        }
    }
